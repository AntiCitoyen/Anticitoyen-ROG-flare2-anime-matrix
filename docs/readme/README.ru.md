...
- **Встроенные обновления**: *Настройки* → *Проверить обновления*; автоматическая проверка раз в день (можно отключить). Программа скачивает `.deb` последнего релиза на GitHub, проверяет контрольную сумму SHA-256 и устанавливает его после запроса пароля администратора (`pkexec`).
