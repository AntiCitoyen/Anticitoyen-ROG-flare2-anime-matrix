...
**Отображение**
- **GIF и изображения**: один файл, выбор файлов или целая папка в виде галереи; потоковое воспроизведение (галерея из 400 GIF умещается в ~25 МБ памяти).
- **Часы** ЧЧ:ММ.
- **19 анимированных эффектов** (дождь в стиле Matrix, плазма, огонь, звёзды, фейерверки, молнии, метаболы, волна, бегущая строка…) и **7 аудиовизуализаторов**, реагирующих на звук, воспроизводимый компьютером.
...
- **Игры** на клавиатуре: Snake, Pong, Tetris, Арканоид, с рекордами.
...
- **Расписание**: интервалы (дни, включая ночь) с часами, галереей, монитором, текущим треком или выключенным экраном; чёрный экран, когда сеанс заблокирован, в спящем режиме или когда приложение развёрнуто на весь экран.
- **Цвета клавиатуры через OpenRGB**: цвет темы на клавишах или пульсация в такт экрану.
...
Протестировано на Ubuntu 26.04 (X11, PipeWire, Cinnamon). Должен подойти любой дистрибутив с Python ≥ 3.10, hidapi, Tk и systemd.
...
| Команды | `animematrix`, `animematrixd`, `animematrix-ctl`, `animematrix-bascule`, `animematrix-animation`, `animematrix-apercu`, `animematrix-convertir`, `animematrix-effet`, `animematrix-galerie`, `animematrix-horloge`, `animematrix-dessin`, `animematrix-tray` |
...
Полезные системные утилиты: `imagemagick` (классическая конвертация), `pulseaudio-utils` (`parec`, для аудио), `zenity` (диалоги выбора файлов), `libnotify-bin` (уведомления), `python3-gi` и `gir1.2-ayatanaappindicator3-0.1` (значок в системном трее), `openrgb` (цвета клавиш).
...
- **GIF / изображения**: *GIF/изображения…* или *Папка (галерея)…*; *Точная геометрия* сохраняет пропорции (угол обрезает изображение вместо растягивания); *👁 Точный предпросмотр (до отправки)* показывает результат, ничего не отправляя; *🎞 Создать анимацию (редактор)*; *📚 Библиотека анимаций*; *Умное преобразование* для конвертации GIF.
- **Эффекты** и **Аудио**: выбрать, настроить, *▶ Запустить эффект*. Ползунки действуют в реальном времени; *Темп* ускоряет или замедляет всю анимацию. В игры играют стрелками, пробелом и Enter, при окне лаунчера на переднем плане.
- **Яркость**, **🕒 Часы**, **■ Остановить** (очищает экран) — общие для всех вкладок.
- **Настройки**: запуск сессии (Галерея GIF, Часы, Последнее воспроизведение или Ничего), язык, тема, интерфейс, уведомления рабочего стола, цвета клавиатуры (OpenRGB), *Расписание…*, значок в системном трее, папка расширений, обновления.
...
### Цвета клавиатуры (OpenRGB)

*Настройки* → *Цвета клавиатуры (OpenRGB)*: цвет темы или пульсация в такт экрану. Демон при необходимости запускает `openrgb --server`. OpenRGB не знает о предыдущей подсветке клавиатуры: чтобы вернуть эффект, сохранённый в клавиатуре, отключите её и подключите снова.
...
| «Keyboard React» не реагирует | сессия Wayland или отсутствует `pynput` | сессия X11, `sudo apt install python3-pynput` |
...
| Клавиши остаются одного цвета после OpenRGB | OpenRGB не воспроизводит исходный эффект | отключить и снова подключить клавиатуру |
...
| `rog_flare2_i18n.py`, `locale/` | перевод (19 языков; `locale/_cles.json` = тексты для перевода) |
| `rog_flare2_demon.py`, `rog_flare2_ctl.py` | демон `animematrixd`, клиент и команда `animematrix-ctl` |
| `rog_flare2_core.py` | потоковое воспроизведение GIF, часы, геометрия |
...
| `rog_flare2_openrgb.py`, `rog_flare2_tray.py`, `rog_flare2_portable.py` | цвета через OpenRGB, значок в системном трее, ноутбуки (экспериментально) |
...
Сообщения об ошибках, идеи и анимации, которыми можно поделиться: [Issues](https://github.com/AntiCitoyen/Anticitoyen-ROG-flare2-anime-matrix/issues).
