...
أدوات نظام مفيدة: `imagemagick` (التحويل الكلاسيكي)، `pulseaudio-utils` (`parec`، للصوت)، `zenity` (منتقيات الملفات)، `libnotify-bin` (الإشعارات)، `python3-gi` وgir1.2-ayatanaappindicator3-0.1 (أيقونة شريط النظام)، `openrgb` (ألوان المفاتيح)، `ffmpeg` (الفيديو، كاميرا الويب، نسخ الشاشة)، `python3-evdev` (الاستجابة للوحة المفاتيح تحت Wayland)، `x11-utils` (النافذة النشطة تحت X11)، `python3-qrcode` (رمز QR للتحكم عن بُعد)، `tkdnd` (السحب والإفلات).
