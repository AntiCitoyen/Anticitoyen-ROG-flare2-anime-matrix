...
- **Графічний лаунчер** (`animematrix`), на вибір із **4 інтерфейсів**: *Циферблат + панель* (кругле вікно з панеллю налаштувань, що виїжджає праворуч, за замовчуванням), *Циферблат* (усе в колі), *Заокруглений* (дуже заокруглені кути, коліщатко яскравості) і *Класичний* (вкладки). Круглі інтерфейси показують **312 світлодіодів наживо**, точно так, як вони надсилаються на клавіатуру. Чотири блоки керування:
  ...
  - **Налаштування**: що показувати при вході в сесію, мова, тема та інтерфейс, редактор малюнка, посилання проєкту.
...
- **Вбудовані оновлення**: *Налаштування* → *Перевірити оновлення*; автоматична перевірка раз на день (можна вимкнути). Програма завантажує `.deb` останнього релізу на GitHub, перевіряє контрольну суму SHA-256 і встановлює його після запиту пароля адміністратора (`pkexec`).
...
- **Налаштування**: *На початку сесії* = Галерея GIF, Годинник, Останнє відтворення або Нічого; *Мова:* змінює мову інтерфейсу (лаунчер перезапускається); *Інтерфейс:* обирає один із 4 інтерфейсів (лаунчер перезапускається, поточне відображення продовжує відтворюватися).
...
| `rog_flare2_i18n.py`, `locale/` | переклад інтерфейсу (19 мов, по одному каталогу JSON на мову) |
| `rog_flare2_themes.py` | теми інтерфейсу (ROG і рожеві) |
| `rog_flare2_ui_ronde.py` | круглі інтерфейси (циферблат + панель, циферблат, заокруглений): малювання, форма вікна, попередній перегляд світлодіодів |
