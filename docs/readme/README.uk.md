...
Корисні системні утиліти: `imagemagick` (класична конвертація), `pulseaudio-utils` (`parec`, для аудіо), `zenity` (діалоги вибору файлів), `libnotify-bin` (сповіщення), `python3-gi` і `gir1.2-ayatanaappindicator3-0.1` (значок у системному треї), `openrgb` (кольори клавіш), `ffmpeg` (відео, вебкамера, дзеркало екрана), `python3-evdev` (реакція на клавіатуру під Wayland), `x11-utils` (активне вікно під X11), `python3-qrcode` (QR-код веб-пульта), `tkdnd` (перетягування).
